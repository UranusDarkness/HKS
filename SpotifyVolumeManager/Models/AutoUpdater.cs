    ...
    public static class AutoUpdater
    {
        ...
        public static void Start(string appCast, Assembly myAssembly = null)
        {
            ...
            if (!Running && _remindLaterTimer == null)
            {
                ...
                _isWinFormsApplication = Forms.Application.MessageLoop;
            ...
            else
            {
                if (result is UpdateInfoEventArgs args)
                {
                    ...
                    else
                    {
                        ...
                        if (ReportErrors)
                        {
                            Forms.MessageBox.Show(Resources.UpdateUnavailableMessage,
                ...
                if (ReportErrors)
                {
                    if (exception is WebException)
                    {
                        Forms.MessageBox.Show(
                            ...
                            Resources.UpdateCheckFailedCaption, Forms.MessageBoxButtons.OK, Forms.MessageBoxIcon.Error);
                    }
                    else
                    {
                        Forms.MessageBox.Show(exception.Message,
                            exception.GetType().ToString(), Forms.MessageBoxButtons.OK,
        ...
        internal static void Exit()
        {
            ...
            else
            {
                if (_isWinFormsApplication)
                {
                    Forms.MethodInvoker methodInvoker = Forms.Application.Exit;
        ...
        public static bool DownloadUpdate(UpdateInfoEventArgs args)
        {
            using (var downloadDialog = new DownloadUpdateDialog(args))
            {
                try
                {
                    downloadDialog.StartDownloadUpdate();
                    return true;
                    //return downloadDialog.ShowDialog().Equals(Forms.DialogResult.OK);
                }
        ...
        public static void ShowUpdateForm(UpdateInfoEventArgs args)
        {
            using (var updateForm = new UpdateForm(args))
            {
                ...
                if (updateForm.ShowDialog().Equals(Forms.DialogResult.OK))
                {
                    Exit();
                }
            }
